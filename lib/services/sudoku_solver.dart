...
class SudokuSolver {
  ...
  static final List<(String, SudokuHint? Function(SudokuBoard))> _hintFinders =
    ...
    ('全双值坟墓+1', AdvancedTechniques.findBugPlusOne),
    ...
    ('染色法', AdvancedTechniques.findSimpleColoring),
    // 唯一环 Type 3 是这一族里最重的一手（7.2），比染色法（7.0）还深，
    ...
    ('强弱交替链', AdvancedTechniques.findAic),
    ('融合式待定数组', AdvancedTechniques.findSueDeCoq),
    ('Nice Loop / AIC 环', AdvancedTechniques.findNiceLoop),
    ('区块链', AdvancedTechniques.findGroupedAic),
    ...
    ('死亡绽放', AdvancedTechniques.findDeathBlossom),
    ...
    // 死亡绽放、Kraken 该出面的局面全抢走——题库 160 题里融合式待定数组
    // 与死亡绽放一次都露不了面。所以按实际力度排在 Kraken 之后。
    ('毛刺数组', AdvancedTechniques.findBurredSubset),
    ('飞鱼导弹', AdvancedTechniques.findExocet),
    ('分类强制链', AdvancedTechniques.findForcingChain),
    ...
    ('分类强制网', AdvancedTechniques.findForcingNet),
  ];
