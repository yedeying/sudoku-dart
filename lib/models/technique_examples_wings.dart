...
List<TechniqueInfo> wingTechniqueExamples() => [
      TechniqueInfo(
        ...
        howToSpot: '选定一个数字，找两列（或两行）候选都恰好剩两格的强链，'
            '再看它们是否有一端共享同一行（或同一列）。',
        walkthrough: '本例中数字 3 在c4的候选只剩r2和r6，'
            '在c7的候选只剩r1和r6，两条强链都在r6有一个端点，'
            '构成屋脊。无论两条链具体怎么取值，r1c6（同时看到r6c4'
            '所在的宫、r1c7所在的行）和r2c8（同时看到r2c4'
            '所在的行、r1c7 所在的宫）都不可能是 3，因此删除这两处的候选 3。\n'
            '3r2c4 = 3r6c4 - 3r6c7 = 3r1c7',
        caveats: '两条强链必须是真正的强链（该数字在那条线上恰好只剩两格），'
            '屋脊只是端点共线，本身不必是强链，不要把普通候选重合误判成摩天楼。',
        rank: 202,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例中数字 9 在r4只能出现在c5和c7，在c6只能出现'
            '在r4和r8，r4c5和r8c6同在一个宫内，构成拐弯。'
            '无论这两个宫内候选具体谁为真，r4c7（既在r4看见一端，'
            '又在 c6 看见另一端）都不可能是 9，因此删除该处候选 9。\n'
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例中数字 4 在右上宫的候选都挤在r4与c9上，形成空矩形；'
            'c3上r4和r8恰好是数字 4 的一条强链。r4把空矩形和这条'
            '强链的一端连起来，因此强链另一端所在的r8，和空矩形所在的c9，'
      ...
      TechniqueInfo(
        ...
        howToSpot: '先找一个双值格当支点，再看它能否连到两个双值格，'
            ...
            '共同的第三个数字。',
        walkthrough: '本例中r9c3是支点，候选 {5, 7}；它与r7c2'
            '（候选 {4, 5}）共享候选 5，又与r9c6（候选 {4, 7}）共享候选 7。'
            '两个翼格都还剩候选 4：支点填 5 时右翼被迫填 4，支点填 7 时左翼被迫填 4，'
            '因此同时看到两翼的 r7c6 不可能是 4，删除该处候选 4。\n'
            '5r9c3 - 5r7c2 = 4r7c2；7r9c3 - 7r9c6 = 4r9c6',
        caveats: '三格必须都恰好是双值格，支点到每个翼格的可见关系（同行/列/宫）'
            '缺一个都不成立，不要把三个候选凑巧重合的格子误判成 XY-Wing。',
        rank: 253,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        howToSpot: '先找一个三值格当支点，再看它能否连到两个双值格，'
            ...
            '（也含 z），三格都共享 z。',
        walkthrough: '本例中r8c4是支点，候选 {1, 4, 5}；它与r8c8'
            '（候选含 4，同行）、r9c5（候选含 4，同宫）相连，三格都共享候选 4。'
      ...
      TechniqueInfo(
        ...
        definition: 'W-Wing 指两个格子的候选完全相同、都是 {a, b}，且存在一条数字 b 的'
            ...
            '必有一端为真，把 b 推到某个双值格上，于是那一格填 b、另一格填 a——两种情况下'
            'a 都会落在某个双值格里，因此同时能看到两个双值格的格子都不可能是 a，可以删除。',
        howToSpot: '先找两个候选完全相同的双值格 {a, b}，再看是否存在一条数字 b 的强链，'
            '两端分别能看到这两个双值格。',
        walkthrough: '本例中r5c6和r4c7的候选都是 {4, 7}；'
            'r6c6与r6c9组成候选 7 的强链，r6c6同列能看到'
            'r5c6，r6c9同宫能看到r4c7。无论强链哪端为真，'
            '数字 4 都会落在两个双值格中的一个，因此同时看到两格的 r4c4, r4c5, r4c6'
            '都不可能是 4，删除这三处候选 4。',
      ...
      TechniqueInfo(
        ...
        howToSpot: '先找一个四值格当支点，再看它是否能连到三个双值格，'
            ...
            '数字，四格还共享同一个数字 z。',
        walkthrough: '本例中r1c1是支点，候选 {3, 4, 7, 9}；r1c2'
            '（候选 {4, 9}）、r2c1（候选 {3, 4}）、r3c3（候选 {4, 7}）'
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例中数字 1 的强链把r1c4、r2c9、r8c5、'
            'r7c3染成颜色 A，把r7c4、r2c5、r8c2、'
            'r5c3染成颜色 B。r5c9同行能看到颜色 B 的r5c3，'
            '同列能看到颜色 A 的r2c9，两种颜色都能看到，因此这里的候选 1 '
            '不可能成立，删除。',
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例中r6c4、r6c5的候选除了 {1, 2} 还都多出'
            '{6, 7}，相当于一个隐藏在矩形里的数对 {6, 7}。这个数对和r6的其它格子'
            '构成显性数对，因此 r6c1, r6c2, r6c8 上和它们重复的候选 6、7 都要删除。',
        caveats: '额外候选去掉矩形本身的 {a, b} 之后才是真正要拼数组的部分，'
            '不要把 {a, b} 也算进数组里，否则会多删或少删。',
        rank: 501,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例中r4c3、r4c9、r5c3、r5c9'
            '的候选都是 {4, 5}，且c9上数字 5 只剩r4和r5这两格能填，'
            '形成强链。5 必然落在这两格之一，为了避免死亡对调，这两格都不能再是 4，'
            '因此删除r4c9、r5c9的候选 4。',
        caveats: '强链必须落在矩形本身所在的行或列上，且矩形四格要先确认真的都是'
            '同样的 {a, b}，否则不能套用唯一矩形 Type 4 的强链结论。',
        rank: 354,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例中r4c8是唯一的三值格，候选是 {2, 5, 6}。'
