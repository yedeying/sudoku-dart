...
const _walsLegend = [
  ...
  TechniqueLegendItem(color: TeachingColors.end, label: '两种情况都删掉的候选'),
];
...
List<TechniqueInfo> advancedSetTechniqueExamples() => [
      TechniqueInfo(
        ...
        caveats: '自噬只是标签，不改变原结构的判定；'
            ...
            '同时落在两条覆盖线上的那一格才删得掉。覆盖允许重叠，基线不允许。',
        rank: 451,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        definition: '弱待定数组（弱ALS，Almost Hidden Set）是隐性一侧的待定数组：'
            '一个区域里挑 N 个数字，'
            '它们在这个区域里的落点合起来恰好 N+1 格。'
            '要是只有 N 格，那就是现成的隐性数组，当场能删；'
            '多出一格，就差一步，于是分成互斥的两种情况——'
            '多出来那一格填的是这 N 个数字之一，还是不是。'
            '两种情况各自往下推，都删掉的候选就是这一页的收获。\n'
            '它和显性一侧的 ALS 是对偶的：ALS 数「N 格上的候选并集有 N+1 个数字」，'
            '弱待定数组数「N 个数字在一个区域里占了 N+1 格」。'
            '另外有人拿 WALS 指 ALS-W-Wing——两朵候选相同的 ALS 靠一条强链相连——'
            '那是显性一侧的东西，和这里讲的不是一回事；'
            'W-Wing、ALS-XZ、ALS-XY-Wing 已经覆盖了那一类显性结构。',
        howToSpot: '在一个区域里挑几个还没填的数字，把它们的候选位置并起来数格数：'
            '数字个数 + 1 就是弱待定数组。'
            '再检查任何真子集都不能已经凑成隐性数组（那一步先做更省事），'
            '而且这几格上的候选并集要大到撑不成一朵 ALS——'
            '不然这张图按显性数组也读得通，说不清「弱」在哪里。',
        walkthrough: '这张盘面上唯余、摒除都已经用尽。观察 c7 上的 1 和 5 这两个数字：'
            ...
            '从多出来的 r2c7 分成两种情况。'
            '情况一：r2c7 填的是 1 或 5。那 r2c7 上的 3、4、6 都得去掉；'
            '4 一走，c7 里的 4 就只剩 r3c7，于是 r3c7=4，'
            '接着 r3 里的 5 只剩 r3c8（r3c8=5），r8 里的 5 只剩 r8c7（r8c7=5），'
            'r8 里的 1 只剩 r8c9（r8c9=1）。\n'
            '情况二：r2c7 填的不是 1 也不是 5。那 1、5 就锁进 r3c7 与 r8c7 两格，'
            '这两格上其余候选都要删掉：r3c7 只剩 5，r8c7 只剩 `{1,5}`；'
            '于是 r3c7=5，r8c7=1。\n'
            '两种情况对照着看：r3c7 情况一填 4、情况二填 5，所以 3r3c7 和 6r3c7 两种情况都没了；'
            'r8c7 情况一填 5、情况二填 1，所以 3r8c7 和 8r8c7 两种情况都没了。'
            '这四个候选可以删，结论是 r3c7 只能填 4 或 5、r8c7 只能填 1 或 5。'
            '（答案里 r2c7=6、r3c7=4、r8c7=5，走的正是情况一。）',
        caveats: '弱待定数组只在「N 个数字恰好占 N+1 格」时成立，'
            '多一格少一格都不算；而且任何真子集都不能已经是隐性数组，'
            '否则先做那一步更直接。'
            '两种情况的推理也必须都走得通：哪一种情况当场矛盾，'
            '那就说明另一种情况直接成立，用不着按弱待定数组讲。'
            '最后不要把它和 ALS-W-Wing 混着说——那一说是显性一侧的两朵 ALS 加一条强链。',
        rank: 653,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        definition: '毛刺数组（Almost Locked Set with a burr）说的是一个数组多出一小块「毛刺」：'
            ...
            '「毛刺为真」那一种情况会沿唯余、摒除推到不能再推为止，'
            '力度与强制链相当，因此提示顺序排在 Kraken 之后，而不是 ALS-XZ 之前。',
        howToSpot: '在一个区域里找 N 格恰好锁 N+1 个数字的待定数组，'
            '再看多出来的那个数字是不是只落在其中一格上——那一格就是毛刺格。'
            '毛刺格自己得还剩三个以上候选，否则「有毛刺还是没毛刺」在那一格上'
            '只是个双值格，谈不上毛刺。'
            '接着分成两种情况算：毛刺为假时数组锁死能删什么，毛刺为真时顺着唯余摒除能删什么，'
            '两种情况的交集就是收获。',
        walkthrough: '这张盘面上唯余、摒除都已经用尽，接着看 r7 上的三格：'
            ...
            '两种情况这样走。情况一：毛刺为假，也就是 r7c8 不填 7。'
            '那么 `{2,4,5}` 被这三格锁死，r7 上别处的这些数字都得让位：'
            '2r7c2、5r7c2、2r7c3、5r7c3、2r7c7、4r7c7、5r7c7 一共七个候选。\n'
            '情况二：毛刺为真，r7c8=7。顺着推：b9 里的 4 本来只在 r7c7、r7c8 两格，'
            'r7c8 被 7 占住，于是 r7c7=4；接着 c7 里的 2 只剩 r9c7，于是 r9c7=2。'
            '这一种情况只在 r7c7 上动手，把 2r7c7、5r7c7 删掉了，'
            '但 4r7c7 反而被填成了真。\n'
            '取交集：只有 2r7c7 与 5r7c7 两种情况都删得掉。'
            '情况一多删的那五个（r7c2、r7c3 上的，以及 4r7c7）都不能算——'
            '4r7c7 更是反例，情况一说它假、情况二说它真。'
        ...
        caveats: '毛刺只能有一块：多出两个以上额外候选就不再是毛刺数组，'
            ...
            '不要只画出数组本身就结束：'
            '必须写清两种情况各推出什么、交集是哪几个，否则得不出结论。'
            '也不要把情况一单独删掉的候选当成结论——本例的 4r7c7 就是反例。',
        rank: 702,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        definition: 'DDS（Distributed Disjoint Subsets，分布式互斥数组）是融合式待定数组的推广。'
            '融合式待定数组只在一个宫和一条线交叉处把数字拆成两堆互不相交的锁定集；'
            'DDS 把这件事摊到三个以上的区域：挑 N 个格子，'
            '它们的候选并集恰好 N 个数字，而且每个数字在这 N 格里的落点'
            '都被某一个区域整个装下——这个区域就是这个数字的「片」。\n'
            'N 个格子各要填一个数，每个数字的片最多消化一格，'
            'N 格配 N 片，正好一格一片，谁也匀不出来。'
            '所以每一片里、结构之外的那个数字都可以删。'
            '这套计数就是 rank 0；片数至少三条，'
            '否则一两个区域装得下的那些情形已经由显性/隐性数组和融合式待定数组讲过了。',
        howToSpot: '挑几个跨区域的空格，先数「格数 = 候选并集的数字个数」；'
            '再对每个数字看它在这几格里的落点能不能被一个区域整个装下。'
            '两条都成立就是 DDS。'
            '还要防两种冒充：全部格子落在同一个区域里（那是显性数组），'
            '以及存在两个区域就能装下每个数字的落点（那是融合式待定数组那一档）。',
        walkthrough: '本例挑六格：c1 上的 r2c1 `{1,5}`、r5c1 `{1,5,8}`、r7c1 `{4,5,8}`，'
            ...
            '于是六个数字分到三个区域上：c1 装 1、5、8，左下宫装 4，r8 装 9、2。'
            '六格六片，rank 0。'
            '这三个区域换不成两条——4 那一片必须跨到左下宫，'
        ...
        caveats: '片与片不能共用格子，每个数字也必须严格落在自己那一片里——'
            ...
            '而如果两个区域就装得下每个数字的落点，'
            '这个图形骨子里是融合式待定数组，不该按「分布到三个以上区域」讲。',
        rank: 750,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        definition:
            '网（MSLS，Multi-Sector Locked Sets，多区域锁定集）是融合式待定数组和 DDS 的更一般形。'
            '挑一批格子，再给每个数字配上「装得下它在结构里全部落点」的区域，'
            '每个区域算一条链接。DDS 要求每个数字只占一条链接，'
            '网放开了这一条：一个数字可以占两个区域、算两条链接。\n'
            '道理还是数链接。每个结构格要填一个数，那个「数字 + 区域」的组合就是一条链接；'
            '两个格子不可能用同一条链接（同区域同数字只能有一个）。'
            '所以格子到链接是单射，链接条数又和格数相等，于是成了双射——'
            '每条链接都被用掉一次。链接被用掉，就意味着这个数字在那个区域里落在结构格上，'
            '于是那个区域里结构之外的同名候选全都可以删。'
            '这就是 rank 0。\n'
            '难处在于「挑哪些格子、配哪些区域」没有任何形状上的线索，'
            '只能靠通用覆盖搜索逐一尝试。'
            '它没有独立于通用覆盖搜索的判定方法，因此只作教学说明，不纳入提示。',
        howToSpot: '先看融合式待定数组和 DDS 这些有固定形状的特例，它们才有得找。'
            '真要认网，就逐个数字配区域、把链接条数和格数摆在一起数，'
            '相等才算 rank 0；顺手检查每个结构格的每个候选都被某条链接覆盖，'
            '漏一个单射就断了。',
        walkthrough: '本例挑六格：r7 上的 r7c4 `{3,7}`、r7c5 `{7,9}`、r7c6 `{3,7,9}`，'
            '还有 c8 上的 r2c8 `{5,9}`、r8c8 `{8,9}`、r9c8 `{8,9}`。'
            '六格里只出现五个数字——`{3,5,7,8,9}`，比格数少一个，'
            '所以光靠「格数 = 数字数」是配不平的，这正是 DDS 做不到、网才能做的地方。\n'
            '逐个数字配区域：3 落在 r7c4、r7c6，一条 r7 就够；'
            '7 落在 r7c4、r7c5、r7c6，也是一条 r7；'
            '8 落在 r8c8、r9c8，一条 c8；5 只落在 r2c8，一条 r2。'
            '9 最特别：它落在 r7c5、r7c6、r2c8、r8c8、r9c8 上，'
            '一个区域装不下，得用 r7 加 c8 两条——它自己就占了两条链接。\n'
            '数一数：r7 的 3、r7 的 7、r7 的 9、c8 的 9、r2 的 5、c8 的 8，'
            '一共六条链接，正好等于六格，rank 0 成立。'
            '所用的区域是 r7、c8、r2 三片，够得上「多区域」。\n'
            '于是每条链接都被用掉一次，各自把区域里结构之外的同名候选清掉：'
        ...
        caveats: '网仅作教学说明，不纳入提示：它没有独立判定方法。'
            '平衡条件算错就会删掉真数，比一般技巧危险得多——'
            '尤其要注意「每个结构格的每个候选都必须被某条链接覆盖」这一条，'
            '漏掉一个，那一格就能跳出结构，整套计数立刻失效。'
            '另外不要把每个数字只占一个区域的图形当网讲，那种正好是 DDS。',
        rank: 850,
        examplePuzzle:
        ...
        exampleMarkup: schematicMarkup(
          ...
          // 9 占两个区域，所以 r7 与 c8 上各画一段，不跨区域连线。
          weakLinks: const [
            [6, 3, 6, 5, 3],
            [6, 4, 6, 5, 7],
            [6, 4, 6, 5, 9],
        ...
        structure: const TeachingStructure(
          ...
          // 9 一个数字吃掉 r7 与 c8 两个区域，这是网比 DDS 一般的地方。
          sectorLinks: [
            SectorLink(3, [6]),
            SectorLink(7, [6]),
            SectorLink(9, [6, 16]),
      ...
      TechniqueInfo(
        ...
        definition: '飞鱼导弹（Exocet）在同一带里取两个同宫同线的基格，'
            ...
            '其中一格带基格数字（目标格），另一格一个基格数字都不带（伴随格，已知数也算）。'
            '三条交叉线是两个目标格所在的那两条，加上基格宫里基格没占的那一条。'
            '交叉线伸出带外的那部分叫 S 格。\n'
            'Junior Exocet（JE）的判定条件是：每个基格数字在 S 格上的全部出现'
            '（候选、已知数都算）都能被不超过两个区域覆盖。'
            '这时基本结论成立——两个目标格必须分别落到两个基格数字上，'
            '于是目标格上不属于基格数字集的候选都可以删。\n'
            '同一枚 JE 还能继续往下推：某个基格数字在 S 格上只被一条房屋盖住'
            '（或完全不出现）时，它当不了真基格数字，基格所在小行和目标格上的它都可以删；'
            '覆盖线本身就是一条交叉线时，该线上的目标格也不能填这个数字；'
            '前面几步让一个目标格只剩一个基格数字时，另一个目标格不能再填它。\n'
            'Senior、Weak、Double、Locked Member 是同一套几何下的其它变体，'
            '镜像格那一路的规则另外计算。',
        howToSpot: '在一条带里找同宫同线的两个空基格，看候选并集是不是三四个数字；'
            '再到另外两宫里找对象格对：一格带基格数字、另一格一个都不带。'
            '最后数覆盖线——每个基格数字在 S 格上的出现都要被两个区域覆盖。'
            '这一步最容易漏，漏了删除就不成立。',
        walkthrough: '本例开局没有更浅的一手，提示第一步就是飞鱼导弹。'
            '基格取右上带里同在 r3 的 r3c1 `{1,2,3}` 与 r3c2 `{1,2,3,4}`，'
            '并集是 `{1,2,3,4}` 四个数字。\n'
            '带是 r1–r3。中上宫里取 c4 这一对：r1c4 已经填了 7，'
            '一个基格数字都不带，是伴随格；r2c4 `{1,2,3,4,5}` 带着基格数字，是目标格。'
            '右上宫里取 c7 这一对：r2c7 填的是 9，同样一个基格数字都没有，是伴随格；'
            'r1c7 `{1,2,3,6}` 是目标格。'
            '于是三条交叉线是 c4、c7，加上基格宫里空着的 c3。\n'
            '数覆盖线。S 格就是 c3、c4、c7 在 r4–r9 上的那些格子。'
            '1 出现于 r4c3、r4c4、r9c3、r9c7——r4 与 r9 两条行就覆盖了；'
            '2 出现于 r6c3、r6c4、r8c3、r8c7——r6 与 r8；'
            '3 出现于 r6c3、r6c7、r8c3、r8c4——同样是 r6 与 r8；'
            '4 出现于 r9c3、r9c4，以及已知数 r5c7——r9 与 r5。'
            '四个基格数字都只需两条覆盖线，JE 条件满足。\n'
            '道理是这样：设基格里真填的两个数是 A、B。'
            '三条交叉线各要放一个 A，一共三个。'
            '这三个 A 落不到基格看得见的逃逸格上，也落不到伴随格上（按定义那里没有基格数字），'
            '所以只能待在目标格或 S 格里。'
            '而 S 格上的 A 全被两条覆盖线覆盖，两条线最多放下两个 A，'
            '于是至少有一个 A 落在目标格上。B 同理。'
            '两个目标格互不相同，只好一个装 A、一个装 B。\n'
            '既然目标格只能填基格数字，它们身上别的候选就都能删：'
            'r2c4 上删 5，r1c7 上删 6。'
            '（答案里 r2c4=4、r1c7=3，正是两个不同的基格数字。）\n'
            '镜像格在这里只标出来备用：r2c4 的镜像是 r1c8、r1c9，'
            'r1c7 的镜像是 r2c5、r2c6——都是「贴着对面那个目标格」的两格。'
            '镜像格还能推出更多删除，本页不展开。',
        caveats: '基格必须同宫同线；两个目标格必须在同一带的另外两宫、'
            '各占不同的交叉线、而且都不在基线上（不然它们看得见基格）。'
            '伴随格「一个基格数字都不带」这一条要把已知数也算进去，'
            '漏算就等于允许一对对象格装下两个基格数字，整套推理塌掉。'
            '覆盖线最多两条也必须逐个数字核对。\n'
            '本例盘面只触发基本结论：目标格上删非基格候选。'
            '覆盖线延伸那几条（单覆盖数字、交叉线当覆盖线、目标格互斥）'
            '在别的残局里会跟着同一枚 JE 一起报。\n'
            '镜像格那一路的规则（Bird 的 Rule 6 到 Rule 9）在文献里还有争议——'
            '照字面实现会删出错的候选，所以这里不用它下结论。'
            'Senior、Weak、Double、Locked Member 各变体也都还没实现。',
        rank: 900,
        examplePuzzle:
            '980700000600000970007000005400030020008600400000004001006800500000010004000002030',
        exampleMarkup: schematicMarkup(
          pattern: const [
            [2, 0],
            [2, 1],
        ...
        structure: const TeachingStructure(
          ...
          baseDigits: {1, 2, 3, 4},
          exocet: ExocetSpec(
            baseCells: [CellRef(2, 0), CellRef(2, 1)],
            targets: [CellRef(1, 3), CellRef(0, 6)],
            companions: [CellRef(0, 3), CellRef(1, 6)],
            mirrors: [
              [CellRef(0, 7), CellRef(0, 8)],
              [CellRef(1, 4), CellRef(1, 5)],
            ],
            // c3、c4、c7。
            crossLines: [11, 12, 15],
            coverLines: [
