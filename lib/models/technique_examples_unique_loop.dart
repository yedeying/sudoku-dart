...
/// 测试会独立复核环的每个区域是不是恰好占两格、六格是不是都含底数对、
...
List<TechniqueInfo> uniqueLoopTechniqueExamples() => [
      TechniqueInfo(
        ...
        caveats: '偶环必须每个区域都恰好占两格，占一格或占四格都不是唯一环，'
            '搜环时最容易在这一步出错。',
        rank: 551,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        definition: '唯一环 Type 2 对应唯一矩形 Type 2：环上恰好有两格除底数之外都多出同一个额外数字 c，'
            '其余格子只剩底数。这两格不可能都把 c 去掉，否则整个环只剩底数，'
            '两个底数沿环交替对调就会造出第二个解。'
            '所以 c 至少在这两格之一为真，同时看得见这两格的位置上，c 可以删。'
            '几何比矩形长，但用法一字不改。',
        howToSpot: '搜到偶环后先数额外候选：'
            '只有两格带额外候选、而且带的是同一个数字，就走 Type 2。',
        walkthrough: '本例的环是 r3c7 → r3c8 → r5c8 → r5c9 → r7c9 → r7c7 → 回到 r3c7，'
        ...
        caveats: '两格多出来的必须是同一个数字；数字不同就得改走 Type 3 的虚拟格，'
      ...
      TechniqueInfo(
        ...
        definition: '唯一环 Type 4 对应唯一矩形 Type 4（锁定型）：环上恰好两格带额外候选，而且这两格'
            '同在一个区域里；如果某个底数在这个区域里只剩环上这两格，它就形成强链。'
            '这个底数一定落在这两格之一，那另一格就只能去填自己多出来的候选，'
            '于是两格各自都只剩「这个底数」和「自己的额外候选」两种可能，'
            '另一个底数就可以从这两格删掉。判定顺序是先搜偶环，再在环经过的区域里找底数强链。',
      ...
      TechniqueInfo(
        id: 'ul3',
        name: '唯一环 Type 3',
        summary: '环上两格的多余候选合成虚拟格，和同区域的格子配数组。',
        definition: '唯一环 Type 3 对应唯一矩形 Type 3：环上有两格除底数之外还各带额外候选，'
            '而且这两格同处一个区域。'
            '环不可能整条都只填底数，所以这两格里至少有一个要填自己的额外候选——'
            '把它们合起来当成一个「虚拟格」，虚拟格的候选就是那些额外数字。'
            '虚拟格算一格，再和同区域其它格子配成数组：'
            'k 个格子（含虚拟格）锁住 k 个数字，这几个数字在这个区域别处就能删。'
            '虚拟格背后是完整性约束，绝不能把额外候选拆开当裸对按裸对直接删除。'
            '这一型的工作量最接近唯一矩形 Type 3。',
        howToSpot: '搜到偶环后找只有两格带额外候选、而且这两格同区域的情形，'
            '把两格的额外候选并成一格，再在这个区域里找能配成数组的格子。',
        ...
        caveats: '虚拟格只能整体参与数组；'
            '配数组的格子必须和虚拟格同处一个区域，格数和锁住的数字个数也要正好相等。',
        rank: 600,
        examplePuzzle:
