...
/// 这样教学页画出来的候选就是推理用的候选，「这个数字在这个房屋里有几个落点」
...
List<TechniqueInfo> advancedChainTechniqueExamples() => [
      TechniqueInfo(
        ...
        definition: '死环（Broken Wing / 全强链奇数圈）盯的是同一个数字：'
            '取奇数个格子首尾相连成一圈，相邻两格同属一个房屋，各条边的房屋互不相同。'
            '每条边所在的房屋里，除了圈上那两格，往往还留着别的落点，'
            '这些落点叫守卫。'
            '先把全部守卫假设为假：那么每条边的房屋里这个数字就只剩圈上两格，'
            '每条边都成了真强链——「两端恰好一真一假」。'
        ...
        howToSpot: '锁定一个数字，找奇数个格子连成的圈，各条边的房屋要互不相同、'
            '而且每条房屋只许占圈上两格；'
            '再把每条边房屋里圈外的同名候选数清楚，那些就是守卫。'
            '守卫越少越好用——同时看得见它们全部的位置就是删除落点。',
        walkthrough: '本例盯数字 9。圈是 r4c4 → r4c8（同在 r4）→ r7c8（同在 c8）'
            '→ r7c6（同在 r7）→ r5c6（同在 c6）→ 回到 r4c4（同在中中宫），'
            '五条边、五个房屋各不相同，每条房屋里也只占了圈上两格，是个合格的奇数圈。\n'
        ...
        caveats: '圈长必须是奇数，各条边的房屋必须互不相同，'
            '而且每条边的房屋里只许有它自己那两个圈上格——'
      ...
      TechniqueInfo(
        ...
        walkthrough: '这张盘面上唯余、摒除都已经用尽。假设 8r1c7 为真，看它能不能站得住。\n'
            ...
            '要填到第十二格才撞出矛盾。动态强链把它压到了「五格 + 两支各不到五格」，'
            '这就是它换来的东西。',
