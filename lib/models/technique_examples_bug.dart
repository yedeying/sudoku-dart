...
List<TechniqueInfo> bugTechniqueExamples() => [
      ...
      TechniqueInfo(
        ...
        definition: 'BUG 类型 4 对应唯一矩形 4（锁定型）：盘面差一点就是双值死盘，'
            '两个例外格落在同一个房屋里，而它们共有的某个底数在这个房屋里只剩这两格，'
      ...
      TechniqueInfo(
        ...
        definition: 'BUG 类型 3 对应唯一矩形 3：盘面差一点就是双值死盘，'
            '两个例外格落在同一个房屋里，各自多出一个不同的额外候选。'
            '因为这两个额外候选至少有一个为真，可以把它们并成一个「虚拟格」，'
            '虚拟格的候选就是那两个额外数字；再拿虚拟格和同房屋里候选正好是这两个数的格子'
            '配成数对，按数组规则删除。虚拟格背后是完整性约束，'
            '不能把它拆开当普通裸对硬删。这一型比类型 2、4 都重。',
      ...
      TechniqueInfo(
        ...
        caveats: '+n 只是个计数，别把 +2、+3 当成独立技巧名；'
