...
List<TechniqueInfo> pendingTechniqueExamples() => [
      TechniqueInfo(
        ...
        howToSpot: '先找四角同宫成对、底数一致的矩形，再数清楚挡路的多余候选；'
            '正好两个而且不构成 Type 2 时，把这两个候选当成链上的一个强链往外接。',
        walkthrough: '本例的矩形是 r2c8、r2c9、r7c8、r7c9，落在右上宫和右下宫，'
            ...
            '这两个候选数字不同、两格又互不相见，'
            'Type 2、Type 4 那种当场删除都用不上，只能停在链节点上：'
            '本页只交出这条强链，删除留给接上它的那条交替推理链去收口。',
        caveats: '待定结构必须先有确凿的矩形几何，多余候选还要数清楚一个不漏；'
            '如果两个多余候选是同一个数字又互相看得见，那就是能直接删的 Type 2，'
            '不该停在链节点上。',
        rank: 703,
        examplePuzzle:
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例 20 个空格里有 18 个是双值格，例外是 r7c1（`{2,3,7}`，底数 `{2,3}`，多出 7）'
            ...
            '每格恰好两个候选、每个区域里每个未填数字恰好出现两次——'
            '所以 7r7c1 与 2r9c4 不可能同时为假。'
            '两格互不相见，直接的 Type 2、4 都用不上，只能把这条约束当链节点：'
        ...
        caveats: '例外格必须数清楚，一个都不能漏，奇偶条件也要逐个区域核对；'
      ...
      TechniqueInfo(
        ...
        caveats: '环上每个区域必须恰好占两格；'
            '而且待定唯一环只提供一条「不能同时为假」的关系，'
            '两个多余候选落在不同区域时，不接链就删不出任何候选。',
        rank: 721,
        examplePuzzle:
