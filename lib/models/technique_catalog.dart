...
class TechniqueCatalog {
  ...
  static List<TechniqueInfo> _build() {
    final items = <TechniqueInfo>[
      _t('naked_single', '唯一候选数', '一格只剩一个数字，直接填。', 10),
      _t('hidden_single', '隐藏单元', '某行/列/宫里某个数字只剩一个位置，填那里。', 20),
      _t('naked_pair', '显性数对', '两格恰好共有两个数字，从同区域其它格删掉它们。', 30),
      _t('naked_triple', '显性三元组', '三格恰好共有三个数字，从同区域其它格删掉它们。', 35),
      _t('naked_quad', '显性四元组', '四格恰好共有四个数字，从同区域其它格删掉它们。', 40),
      _t('hidden_pair', '隐性数对', '两个数字只出现在同样两格，这两格删掉其它候选。', 45),
      _t('hidden_triple', '隐性三元组', '三个数字只出现在同样三格，这三格删掉其它候选。', 50),
      _t('hidden_quad', '隐性四元组', '四个数字只出现在同样四格，这四格删掉其它候选。', 55),
      _t('pointing', '指向对', '宫内某数字全挤在同一行或列，删这行/列宫外的该数字。', 60),
      _t('box_line', '盒线削减', '行/列上某数字全挤在同一宫，删这宫里其它行/列的该数字。', 65),
      ...
      _t('ur4', '唯一矩形 Type 4', '题目保证唯一解：矩形所在线上一数字形成共轭，删矩形内另一个数字。', 165),
