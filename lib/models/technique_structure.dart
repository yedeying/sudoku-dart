...
enum TeachingFamily {
  ...
  /// 可规避矩形：几何同唯一矩形，但要求角上已填的数字是玩家推出来的，不是给定数。
  avoidableRect,
...
enum TeachingClaim {
  ...
  /// 淑芬类型 1：线外两格只有一格带额外候选，那一格必须跳出底数，
  /// 于是它上面的全部底数都能删掉。
  qiuType1,

  /// 双值死盘类型 2：恰好两个例外格多出同一个数字，
  /// 删同时看得见这两格的位置上的那个数字。
  ///
  ...
  /// 双值死盘 +n：n（≥3）个例外格多出同一个数字，删法同类型 2。
  gravePlusN,

  /// 双值死盘类型 3：两个例外格同房屋、各多出一个不同数字，
  /// 并成虚拟格后与 [TeachingStructure.subsetCells] 配成数组再删。
  graveType3,

  /// 双值死盘类型 4：两个例外格同房屋，共有底数 [TeachingStructure.lockDigit]
...
class TeachingStructure {
  ...
  /// 类型 4 一个房屋，隐性唯一矩形两个房屋（对角格所在的行与列）。
  final List<int> lockHouses;

  /// 类型 3：和虚拟格配成数组的其它格子，以及数组占住的数字。
