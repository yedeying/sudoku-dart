...
List<TechniqueInfo> extendedRectTechniqueExamples() => [
      TechniqueInfo(
        ...
        definition: '扩展矩形（Extended Rectangle）把唯一矩形的两行两列撑成两条线各三格，'
            ...
            'Type 1 是最直接的用法：六格里只有一格比底数多出一个候选，'
      ...
      TechniqueInfo(
        ...
        definition: '扩展矩形 Type 2 对应唯一矩形 Type 2：六格结构里有两格除了底数之外都多出同一个'
            '额外数字 c，其余四格只剩底数。这两格不可能都把 c 去掉——'
            '否则六格只剩底数，两条线上的底数整块对调就多出一个解。'
            '所以 c 至少在这两格之一为真，同时看得见这两格的位置上，c 可以删。'
            '用法和唯一矩形 Type 2 一模一样，只是几何从四格换成六格。',
        howToSpot: '认出六格扩展矩形之后，数一数额外候选：'
            '如果只有两格多出候选、而且多出来的是同一个数字，就走 Type 2。',
        ...
        caveats: '两格多出来的必须是同一个数字，多出两个不同数字就得改走 Type 3 的虚拟格；'
      ...
      TechniqueInfo(
        id: 'er4',
        name: '扩展矩形 Type 4',
        summary: '底数之一被锁在垂直区域的两格上，删这两格的其它底数。',
        definition: '扩展矩形 Type 4 对应唯一矩形 Type 4（锁定型）。'
            '六格结构横着看是三条「垂直区域」，每条上各有两格；'
            'Type 4 要的就是某个底数在其中一条垂直区域里只剩结构上的这两格，'
            '也就是形成一条强链。'
            '这个底数既然一定落在这两格之一，'
            '那另外两个底数如果还完整留在同样这两格上，六格就凑成了可以整块对调的死结，'
            '所以这两格上除强链底数以外的底数都能删。'
            '注意强链只可能出现在只占两格的垂直区域上——'
            '结构自己那两条三格线上，底数本来就要在三格里各占一次，锁不出强链。',
        howToSpot: '认出六格几何后，只看那三条各占两格的垂直区域：'
            '逐个底数数它在这个区域里的落点，看有没有哪个底数只剩结构的那两格。',
        walkthrough: '本例的六格是 c2 上的 r1c2、r2c2、r3c2 和 c8 上的 r1c8、r2c8、r3c8，'
            '落在左上宫和右上宫两个宫里，六格都含底数 `{4,6,9}`。'
            '三条垂直区域是 r1、r2、r3，各占两格。'
        ...
        caveats: '强链要按当前候选现算；'
            '而且不要把结构自己的三格线当成可以锁的区域，那条线上底数各占一次，锁不出强链。',
        rank: 502,
        examplePuzzle:
      ...
      TechniqueInfo(
        id: 'er3',
        name: '扩展矩形 Type 3',
        summary: '两格的多余候选合成虚拟格，和同区域的格子配数组。',
        definition: '扩展矩形 Type 3 对应唯一矩形 Type 3：六格里有两格除底数之外还各带额外候选，'
            '而且这两格同处一个区域。'
            '六格不可能都只填底数，所以这两格里至少有一个要填自己的额外候选——'
            '把它们合起来看成一个「虚拟格」，虚拟格的候选就是那些额外数字。'
            '虚拟格算一格，再拿它和同区域其它格子配成数组：'
            'k 个格子（含虚拟格）锁住 k 个数字，这几个数字在这个区域别处就能删。'
            '这里的关键是虚拟格代表的是完整性约束，不能拆开当普通裸对用。'
            '要留意几何：带额外候选的两格必须真的同区域，'
            '在 2×3 的扩展矩形里只有落在同一条三格线（或同一个宫）上才行。',
        howToSpot: '认出六格几何后，找出只有两格带额外候选、而且这两格同区域的情形，'
            '把它们的额外候选并起来当一格，再在这个区域里找能配成数组的格子。',
