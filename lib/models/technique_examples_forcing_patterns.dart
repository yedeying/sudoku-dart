...
/// 每支只沿唯一后果（某格只剩一个候选、某数字在某房屋只剩一个位置）往下推，
/// 各支都推得出来的结论才能落笔。
...
List<TechniqueInfo> forcingPatternTechniqueExamples() => [
      TechniqueInfo(
        ...
        walkthrough: '本例的矩形是 r4c3、r4c7、r6c3、r6c7，落在左中宫和右中宫，'
            ...
            '第一支假设 2r4c7：r4c7=2 → r5c2=2（左中宫摒除）→ r5c9=3（r5 摒除）'
            '→ r7c9=1（唯余）→ r2c8=1（r2 摒除）→ r6c7=1（r6 摒除）。'
            '第二支假设 3r6c3：r6c3=3 → r5c9=3（右中宫摒除）→ r7c9=1（唯余）'
            '→ r2c8=1（r2 摒除）→ r6c7=1（r6 摒除）→ r7c7=5（唯余）。'
            '第三支假设 1r6c7：这一支开门就是 r6c7=1。'
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例的六格是 c5 上的 r4c5、r5c5、r6c5 和 c9 上的 r4c9、r5c9、r6c9，'
            ...
            '所以这三个候选里必有一个为真。'
            '第一支假设 1r4c9：r4c9=1 → r7c8=1（c8 摒除）→ r9c3=1（c3 摒除）'
            '→ r8c4=1（c4 摒除）→ r2c5=1（c5 摒除）→ r2c6=6（r2 摒除）。'
            '第二支假设 3r6c9：r6c9=3 → r9c7=3（右下宫摒除）→ r9c9=2（r9 摒除）'
            '→ r7c9=6（c9 摒除）。'
            '第三支假设 4r4c5：r4c5=4 → r7c4=4（r7 摒除）→ r8c4=1（c4 摒除）'
            '→ r2c5=1（c5 摒除）→ r2c6=6（r2 摒除）→ r7c3=9（r7 摒除）'
            '→ r9c3=1（c3 摒除）→ r7c2=5（r7 摒除）。'
            '第一支和第三支都把 1 钉在 r9c3 上，第二支干脆把 r9c9 填成了 2——'
            '三种情况下 r9c9 都不可能是 1，'
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例的环是 r3c3 → r3c9 → r1c9 → r1c6 → r2c6 → r2c3 → 回到 r3c3，'
            ...
            '第一支假设 3r3c3：r3c3=3 → r3c2=7（唯余）→ r2c2=2（唯余）'
            '→ r7c1=2（r7 摒除）→ r5c1=1（c1 摒除）→ r8c8=2（r8 摒除）。'
            '第二支假设 8r1c9：r1c9=8 → r2c5=8（中上宫摒除）→ r3c5=1（c5 摒除）'
            '→ r2c2=2（r2 摒除）→ r3c2=7（c2 摒除）→ r3c3=3（r3 摒除）。'
            '第三支假设 9r2c6：r2c6=9 → r1c6=4（c6 摒除）→ r5c6=5（c6 摒除）'
            '→ r8c1=4（c1 摒除）→ r1c1=5（c1 摒除）→ r7c1=2（c1 摒除）。'
            '三支走的是三条不同的路，可是 2 最后都被逼到 r2c2 或 r7c1 上'
            '（第三支干脆把 r1c1 填成了 5）。这两格都同时看得见 r1c1 和 r7c2，'
            '所以 2r1c1 与 2r7c2 三支都删得掉：'
            'r1c1 从 `{2,4,5}` 缩到 `{4,5}`，r7c2 从 `{1,2,6,8}` 缩到 `{1,6,8}`。',
        caveats: '偶环的房屋条件要逐条核对：每一条用到的行、列、宫都必须恰好占两格；'
