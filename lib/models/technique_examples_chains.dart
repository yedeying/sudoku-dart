...
BoardMarkup _chainMarkup({
  ...
  return BoardMarkup(
    ...
    candidateColors: {
      for (final e in eliminated) _cr(e[0], e[1], e[2]): TeachingColors.elimCand,
    },
    arrows: arrows,
  );
}
...
List<TechniqueInfo> chainTechniqueExamples() => [
      TechniqueInfo(
        ...
        walkthrough: '本例中第 1 行第 8 列候选 {4, 6}，与第 2 行第 7 列候选 {5, 6} 因'
            '同宫的候选 6 相连；第 2 行第 7 列的另一候选 5，又与第 9 行第 7 列候选 '
            '{4, 5} 因同列的候选 5 相连。链首第 1 行第 8 列与链尾第 9 行第 7 列都'
            '共享候选 4：若链首不是 4，则它是 6，逼第 2 行第 7 列不是 6、只能是 5，'
            '再逼第 9 行第 7 列不是 5、只能是 4——无论链首是不是 4，4 总会落在'
            '链首或链尾之一。因此同时看到这两格的第 1 行第 7 列不可能是 4，删除该处'
            '候选 4。',
      ...
      TechniqueInfo(
        ...
        definition: 'AIC（交替推理链，Alternating Inference Chain）不要求链上每一格都是'
            ...
            '候选点（同一个数字）就至少有一个为真——因为一旦链首为假，强链会逼出'
            '它相连的下一点为真，再由弱链、强链一路传递，最终逼出链尾为真。因此同时'
            '能看到链首和链尾的格子，这个数字都不可能出现，可以删除。',
        howToSpot: '从任意一个候选点出发，按强链、弱链交替向外延伸，留意每一步连接'
            '所属的单元（同格、同行、同列或同宫），检查首尾两条连接是否都是强链。',
        walkthrough: '本例中第 4 行第 9 列候选 1 与第 5 行第 9 列候选 1 因第 9 列只剩'
            '这两格能填 1 构成强链；第 5 行第 9 列再与第 5 行第 3 列因同一行还有别的'
            '候选 1 构成弱链；第 5 行第 3 列又与第 6 行第 3 列因第 3 列只剩这两格能填 1 '
            '构成强链。链首第 4 行第 9 列与链尾第 6 行第 3 列都是候选 1，且首尾都是'
            '强链连入，因此两者至少有一个是 1。同时看到两端的第 6 行第 7 列不可能是 '
            '1，删除该处候选 1。',
      ...
      TechniqueInfo(
        ...
        howToSpot: '找到强弱交替的链后，看它是否绕回了出发点附近的候选点，'
            ...
            '子链的两端。',
        walkthrough: '本例中数字 2 在第 7 行第 6 列与第 2 行第 6 列因第 6 列只剩这两格'
            '能填 2 构成强链，第 2 行第 6 列再与第 2 行第 4 列因同一行还有别的候选 2 '
            '构成弱链，第 2 行第 4 列又与第 9 行第 4 列因第 4 列只剩这两格能填 2 构成'
            '强链，第 9 行第 4 列最终再通过同宫的弱链绕回第 7 行第 6 列，形成一个环。'
            '取「强-弱-强」这一段子链，两端第 7 行第 6 列与第 9 行第 4 列至少有一个'
            '是 2，因此同时看到这两端的第 9 行第 5 列不可能是 2，删除该处候选 2。',
      ...
      TechniqueInfo(
        ...
        howToSpot: '先找一个数字在某条线上恰好剩三格的候选，看是否有两格恰好同宫，'
            ...
            '那一格。',
        walkthrough: '本例中数字 2 在第 2 列上只剩第 7、8、9 行第 2 列能填，其中第 8 行'
            '第 2 列与第 9 行第 2 列同在左下宫，捆成一组；第 7 行第 2 列则单独构成'
            '另一个节点。列上只剩这三格能填 2，所以「这一组」和「第 7 行第 2 列」'
            '至少有一个是 2。左下宫里第 7、8、9 行第 1 列都能同时看到这个组（同宫）'
            '和第 7 行第 2 列（第 7 行第 1 列还同行），因此这三格都不可能是 2，'
            '删除这三处候选 2。',
      ...
      TechniqueInfo(
        ...
        definition: '几乎锁定集（ALS）指同一行、列或宫里 N 个空格恰好只剩 N+1 个候选'
            ...
            '一个 ALS 里——填了 X 的那个 ALS 会用掉一个候选名额，逼另一个 ALS 里剩下'
        ...
        walkthrough: '本例中第 4 行第 2、3 列组成候选 {1, 7, 9} 的 ALS A，第 5 行第 1、'
            '2 列与第 6 行第 2 列组成候选 {1, 6, 7, 8} 的 ALS B。两者共享候选 7 与 1：'
            'A 里唯一的候选 7 落在第 4 行第 2 列，B 里唯一的候选 7 落在第 5 行第 2 '
            '列，两格同列，互相可见，7 可以作限制公共候选 X。无论 7 最终填在 A 还是'
            'B，另一边都会被逼成真正的锁定集，候选 1 必然落在其中。因此同时看到 A、'
            'B 里所有候选 1 的第 6 行第 3 列不可能是 1，删除该处候选 1。',
      ...
      TechniqueInfo(
        ...
        definition: 'ALS-XY-Wing 是 XY-Wing 的 ALS 版本：支点 ALS 里有两个数字 X、Y，'
            ...
            '或 Y 中的一个名额：用掉 X 时逼翼 A 填满剩下的候选（包含 Z），用掉 Y 时逼'
            '翼 B 填满剩下的候选（也包含 Z）——无论哪种情况，Z 都会落在某个翼 ALS '
            '里，因此同时能看到两个翼 ALS 里所有 Z 候选的格子，Z 都不可能出现，'
            '可以删除。',
        howToSpot: '先找一个几乎锁定集当支点，看它是否能用两个不同的数字，各自和另'
            '一个几乎锁定集构成限制公共候选，再看这两个翼 ALS 是否还共享第三个数字。',
        walkthrough: '本例中第 4 行第 5、6 列组成候选 {1, 4, 7} 的支点 ALS，它用候选 1 '
            '连到第 5 行第 5 列与第 6 行第 4 列组成的候选 {1, 5, 7} 翼 A，又用候选 4 '
            '连到第 4 行第 9 列与第 8 行第 9 列组成的候选 {4, 5, 7} 翼 B。两个翼 ALS '
            '还共享候选 5：支点填 1 时逼翼 A 用掉 5、7；支点填 4 时逼翼 B 用掉 5、7。'
            '无论哪种情况，5 都会落在某个翼 ALS 里，因此同时看到两翼所有候选 5 的'
            '第 8 行第 4 列不可能是 5，删除该处候选 5。',
      ...
      TechniqueInfo(
        ...
        howToSpot: '先找一个宫与一行（或列）的交叉处，数一数交叉格候选总数是否比'
            ...
            '覆盖的几乎锁定集。',
        walkthrough: '本例中第 2 行第 6 列与第 3 行第 6 列是中上宫与第 6 列的交叉格，'
            '候选合起来是 {1, 2, 4, 9}。把 {2, 4} 分给宫：第 2 行第 4 列候选正好是 '
            '{2, 4}，覆盖这一堆；把 {1, 9} 分给列：第 4 行第 6 列候选正好是 {1, 9}，'
            '覆盖另一堆。因此候选 2、4 不会再出现在这个宫的其它格子里，第 3 行第 4 '
            '列的候选 2、4 都要删除；候选 1、9 也不会再出现在第 6 列的其它格子里，'
            '第 5 行第 6 列的候选 9、第 8 行第 6 列的候选 1 都要删除。',
      ...
      TechniqueInfo(
        ...
        definition: 'Death Blossom（死亡花蕾）以一个「花心」格子出发：花心有几个候选'
            ...
            '所有含这个候选的格子。花心最终会填某一个候选，逼对应的花瓣把这个候选'
        ...
        howToSpot: '先找一个候选不算多的花心格子，为它的每个候选分别找一个能被花心'
            ...
            '同一个数字。',
        walkthrough: '本例中第 7 行第 2 列是花心，候选 {3, 5, 9}。候选 3 连到第 7 行'
            '第 6 列（候选 {1, 3}）这个花瓣；候选 5 连到第 8 行第 1 列与第 9 行第 3 '
            '列（候选 {1, 5, 9}）这个花瓣；候选 9 连到第 7 行第 4 列（候选 {1, 9}）'
            '这个花瓣。花心填 3、5 或 9 中的任意一个，对应花瓣排除这个候选后都会'
            '挤出候选 1。因此同时看到这三个花瓣里候选 1 所在格子的第 8 行第 5 列'
            '不可能是 1，删除该处候选 1。',
      ...
      TechniqueInfo(
        ...
        howToSpot: '先找一个数字在某条线上只剩两格能填的强链，看删除目标能不能被'
            ...
            '往下推，看是否也能推到同一个删除目标。',
        walkthrough: '本例中数字 2 在第 7 行只剩第 4 列和第 9 列能填，构成强链。'
            '若第 7 行第 4 列是 2，它与第 4 行第 4 列同列，直接让第 4 行第 4 列'
            '不能是 2；若第 7 行第 9 列是 2，则第 2 行第 8 列成为第 2 行剩下候选 2 '
            '的唯一格、必须是 2，接着第 2 行第 6 列成为剩下候选 9 的唯一格、必须是 '
            '9，最终第 4 行第 4 列成为第 4 行剩下候选 6 的唯一格、必须是 6。两条路径'
            '都逼出第 4 行第 4 列不能是 2，因此删除该处候选 2。',
      ...
      TechniqueInfo(
        ...
        walkthrough: '本例中第 3 行第 4 列候选 {3, 8}，假设它是 3。3 从第 4 列排除后，'
            '陆续用唯一余数推出第 3 行第 1 列是 5、第 7 行第 8 列是 6、第 8 行第 3 '
            '列是 2，接着第 3 行第 9 列是 6、第 7 行第 9 列是 7、第 8 行第 8 列是 9，'
            '再推出第 8 行第 1 列是 1。这时第 8 行已经填了 1、9，第 8 行第 3 列又'
            '填了 2，加上假设本身占掉列上的 3，第 8 行第 4 列原本候选 {1, 2, 3} 被'
            '连续排除后一个不剩，出现矛盾。因此最初的假设错误，第 3 行第 4 列的'
            '候选 3 必须删除，只能填 8。',
      ...
      TechniqueInfo(
        ...
        howToSpot: '挑一个候选不多的格子，让每个候选分别往下推导，'
            '看是否存在某个格子在所有推导路径里都被逼成同一个数字。',
        walkthrough: '本例中第 4 行第 2 列候选 {6, 7}。假设它是 6：唯一余数推出第 6 '
            '行第 2 列是 7、第 5 行第 5 列是 7、第 5 行第 9 列是 6、第 6 行第 5 列是 '
            '6，接着第 9 行第 2 列是 4、第 1 行第 4 列是 4、第 9 行第 4 列是 6。'
            '假设它是 7：同样推出第 6 行第 2 列是 6、第 5 行第 5 列是 6、第 5 行第 9 '
            '列是 7、第 6 行第 5 列是 7，接着第 9 行第 2 列还是 4、第 1 行第 4 列还是'
            ' 4、第 9 行第 4 列还是 6。两条路径都推出第 9 行第 2 列必须是 4，'
            '因此第 9 行第 2 列直接填 4，候选 6、7 都要删除。',
      ...
      TechniqueInfo(
        ...
        howToSpot: '挑一个候选数偏多的格子，让每个候选分别展开推导，'
            '哪怕某条路径要走更多步，只要最终都能找到同一个格子被逼成同一个数字'
            '即可。',
        walkthrough: '本例中第 3 行第 2 列候选 {1, 7, 9}。假设它是 1：唯一余数很快'
            '推出第 3 行第 9 列是 9、第 2 行第 8 列是 1、第 3 行第 1 列是 7，'
            '接着第 5 行第 3 列成为本行剩下候选 1 的唯一格，必须是 1。假设它是 7：'
            '推出第 3 行第 1 列是 9、第 3 行第 9 列是 1、第 2 行第 2 列是 1，'
            '第 5 行第 3 列同样被逼成 1。假设它是 9：推出第 3 行第 1 列是 7、'
            '第 3 行第 9 列是 1、第 2 行第 2 列是 1、第 4 行第 8 列是 1，'
            '第 5 行第 3 列还是被逼成 1。三条网络都汇到第 5 行第 3 列必须是 1，'
