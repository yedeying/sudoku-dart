...
void main() {
  group('候选数功能测试', () {
    ...
    test('候选数模式下填入数字', () {
      var gameState = GameState();
      gameState.loadExampleGame('easy');
      ...
      // 填入候选数字
      gameState.placeNumber(1);
      gameState.placeNumber(2);
      gameState.placeNumber(4);
      
      var userCands = gameState.board!.getUserCandidates(0, 2);
      print('候选数模式填入的数字: ${userCands.toList()..sort()}');
      expect(userCands.length, 3);
      expect(userCands.contains(1), true);
      expect(userCands.contains(2), true);
      expect(userCands.contains(4), true);
      
      // 再次点击应该移除
      gameState.placeNumber(2);
      expect(gameState.board!.getUserCandidates(0, 2).contains(2), false);
      expect(gameState.board!.getUserCandidates(0, 2).length, 2);
