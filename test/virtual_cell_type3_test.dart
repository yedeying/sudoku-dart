...
void _walkBank(
  void Function(SudokuBoard board, String puzzle) inspect, {
  void Function(String puzzle)? onPuzzle,
}) {
  for (final puzzle in loadBank()) {
    onPuzzle?.call(puzzle);
    final board = SudokuBoard.fromString(puzzle);
    for (var step = 0; step < 200; step++) {
      if (board.isComplete()) break;
      inspect(board, puzzle);
...
void main() {
  ...
  test('例外格自己：虚拟格里的候选一个都不许删，虚拟格外的数组数字可以删', () {
    var ownerElims = 0;
    late SudokuBoard solved;
    _walkBank(onPuzzle: (puzzle) {
      solved = SudokuBoard.fromString(puzzle);
